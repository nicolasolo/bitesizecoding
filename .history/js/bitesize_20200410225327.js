...
var i;
  ...
  xmlhttp.onreadystatechange = function() {
    if (this.readyState == 4 && this.status == 200) {
      ...
      var itemssOnPage = [];//arrays for pages
      console.log(itemssOnPage);
      for (i = 0; i < Math.ceil(items.length/itemsPerPage); i++){
        itemssOnPage[i] = items.slice((i*itemsPerPage), (i*itemsPerPage) + itemsPerPage);
        $('<a>', { href: '?page='+(i+1)+'', text: ''+(i+1)+''}).appendTo(pagination);
      }
      showData(itemssOnPage[currentPage-1]);
    }
  };
